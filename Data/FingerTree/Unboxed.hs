...
$(declareType "defineNode" [d| data Node v a = Node2 !v a a | Node3 !v a a a |])

deriving instance (Show v, Show a) => Show (Polymorphic (Node v a))

instance Unpacked (Node v a) => Foldable (Node v) where
	foldMap f (unMk -> Node2 _ a b) = f a `mappend` f b
	foldMap f (unMk -> Node3 _ a b c) = f a `mappend` f b `mappend` f c
        {-# INLINABLE foldMap #-}

node2        ::  (Unpacked (Node v a), Measured v a) => a -> a -> Node v a
node2 a b    =   mk $ Node2 (measure a `mappend` measure b) a b
{-# INLINABLE node2 #-}

node3        ::  (Unpacked (Node v a), Measured v a) => a -> a -> a -> Node v a
node3 a b c  =   mk $ Node3 (measure a `mappend` measure b `mappend` measure c) a b c
{-# INLINABLE node3 #-}
